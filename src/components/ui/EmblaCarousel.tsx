...
type PropType = {
  ...
  symptomsCardsData: SymptomsCardData[];
};

const EmblaCarousel: React.FC<PropType> = (props) => {
  const { symptomsCardsData, slides, options } = props;
    ...
    <div className="embla">
      <div className="embla__viewport" ref={emblaRef}>
        <div className="embla__container">
          ...
          {symptomsCardsData.map((symptom, index) => (
            <div key={index} className="embla__slide">
              <div className="embla__slide__number">
                <div className="mb-1 mt-3 text-sm font-normal uppercase">
                  {symptom.title}
                </div>
                <Image
                  src={`/images/${symptom.url}`}
                  alt={`symptom.title Image`}
                  width={183}
                  height={110}
                />
                <Button className="mt-4 h-6 w-3/4 text-xs font-bold">
                  Need Help
                </Button>
                <Button
                  variant="outline"
                  className="mt-2 h-6 w-3/4 text-xs font-bold"
                >
                  Learn More
                </Button>
              </div>
          ...
          {scrollSnaps.map((_, index) => (
            <DotButton
              ...
              className={"m-2 h-4 w-4 rounded-full bg-brandDark".concat(
                index === selectedIndex
                  ? "h-4 w-4 rounded-full bg-brandPrimary"
                  : "",
              )}
            />
          ))}
